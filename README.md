...
## ⚡ Quick Start
To initialize the network and spin up the complete data architecture in the background, run:
```bash
docker compose up -d
```
...
## ⚡ Quick Complete Stop
To safely tear down the environment while preserving your volume data:
```bash
docker compose down
```
